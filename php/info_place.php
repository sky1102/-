...
<body style=" font-family: 微軟正黑體;">
    ...
    <div class="px-5 mt-4 col-xs-10 col-sm-10 col-md-10 col-lg-10">
      ...
      <?php
        ...
        $farm_ID = $_GET['id'];
        $sql = "SELECT * FROM place WHERE farm_ID='{$farm_ID}'";  // set up your sql query
        $result = $conn->query($sql); // Send SQL Query               
    
        if ($result->num_rows > 0) {  
          $row = mysqli_fetch_array ( $result ,MYSQLI_ASSOC);
          //將空白還原
          $row["intro"]=str_replace(" ","&nbsp;",$row["intro"]);
          //將換行還原
          $row["intro"]=nl2br($row["intro"]);
          echo "
           <div class='frame'>
              <div class='state'>
                <b style='font-size:25px'>　產地：</b>".$row["country"]."</div><br>
              <div class='state'>
                <b style='font-size:25px'>　位置：</b>".$row["state"]."</div><br>
              <div class='state'> 
                <b style='font-size:25px'>　介紹：</b><div class='intro'>　　".$row["intro"]."</div></b>
              </div>
           </div>
          ";
        } 
